...
def do_reordering_check(map_path: Path, expected_map_path: Path, *, print_all: bool=False, reverse_check: bool=True) -> int:
    ...
    mapfile_parser.frontends.bss_check.printSymbolComparison(comparison_info, print_all)
...
def check_reordering_main():
    ...
    BUILTMAP = build_folder / args.version / f"puzzleleague64.{args.version}.map"
    EXPECTEDMAP = "expected" / BUILTMAP

    exit(do_reordering_check(BUILTMAP, EXPECTEDMAP))
